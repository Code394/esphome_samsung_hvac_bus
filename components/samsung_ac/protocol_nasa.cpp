...
#define LOG_MESSAGE(message_name, temp, source, dest)                                                         \
    ...
    {                                                                                                         \
        ...
        */
        ...
        // Helper method to build a Packet from a ProtocolRequest
        void NasaProtocol::build_packet_from_request(Packet& packet, const ProtocolRequest& request)
        {
            // Set up the packet fields based on the request
            // (This logic should be the same as what was previously duplicated)
            // Example (replace with actual logic as needed):
            if (request.mode)
            {
                MessageSet mode_msg(MessageNumber::ENUM_mode);
                mode_msg.value = static_cast<uint8_t>(request.mode.value());
                packet.messages.push_back(mode_msg);
            }
            if (request.power)
            {
                MessageSet power_msg(MessageNumber::ENUM_power);
                power_msg.value = request.power.value() ? 1 : 0;
                packet.messages.push_back(power_msg);
            }
            if (request.automatic_cleaning)
            {
                MessageSet auto_clean_msg(MessageNumber::ENUM_automatic_cleaning);
                auto_clean_msg.value = request.automatic_cleaning.value() ? 1 : 0;
                packet.messages.push_back(auto_clean_msg);
            }
            if (request.water_heater_power)
            {
                MessageSet wh_power_msg(MessageNumber::ENUM_water_heater_power);
                wh_power_msg.value = request.water_heater_power.value() ? 1 : 0;
                packet.messages.push_back(wh_power_msg);
            }
            if (request.target_temp)
            {
                MessageSet temp_msg(MessageNumber::ENUM_target_temp);
                temp_msg.value = request.target_temp.value();
                packet.messages.push_back(temp_msg);
            }
            if (request.water_outlet_target)
            {
                MessageSet water_outlet_msg(MessageNumber::ENUM_water_outlet_target);
                water_outlet_msg.value = request.water_outlet_target.value();
                packet.messages.push_back(water_outlet_msg);
            }
            if (request.target_water_temp)
            {
                MessageSet water_temp_msg(MessageNumber::ENUM_target_water_temp);
                water_temp_msg.value = request.target_water_temp.value();
                packet.messages.push_back(water_temp_msg);
            }
            if (request.fan_mode)
            {
                MessageSet fan_mode_msg(MessageNumber::ENUM_fan_mode);
                fan_mode_msg.value = static_cast<uint8_t>(request.fan_mode.value());
                packet.messages.push_back(fan_mode_msg);
            }
            if (request.alt_mode)
            {
                MessageSet alt_mode_msg(MessageNumber::ENUM_alt_mode);
                alt_mode_msg.value = static_cast<uint8_t>(request.alt_mode.value());
                packet.messages.push_back(alt_mode_msg);
            }
            if (request.swing_mode)
            {
                MessageSet hl_swing(MessageNumber::ENUM_in_louver_hl_swing);
